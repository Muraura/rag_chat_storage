...
from src.db_util.models import ChatSession
...
@router.get("/", dependencies=[Depends(get_api_key), Depends(limiter.limit("10/minute"))])
async def create_chat_session(payload: dict, db: Session = Depends(get_db)):
    print("here we are")
    validation_list = [
    ...
    try:
        ...
        logger.error("#chat.py #create_chat_session #Exception: {}".format(e), exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to create chat session")



@router.patch("/{session_id}", dependencies=[Depends(get_api_key), Depends(limiter.limit("10/minute"))])
async def update_chat_session(session_id: str, payload: dict, db: Session = Depends(get_db)):
    if not payload:
    ...
    try:
        ...
        updated = await mysql_query_utils.update_field_details(
            db, ChatSession, query_object={"id": session_id}, update_object=payload
        )
...
@router.delete("/{session_id}", dependencies=[Depends(get_api_key), Depends(limiter.limit("10/minute"))])
async def delete_chat_session(session_id: str, db: Session = Depends(get_db)):
    try:
        await mysql_query_utils.delete_field_details(
...
async def list_chat_sessions(user_id: str = None, db: Session = Depends(get_db)):
    try:
