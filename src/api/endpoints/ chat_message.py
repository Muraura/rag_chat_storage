...
@router.post("/", dependencies=[Depends(get_api_key), Depends(limiter.limit("20/minute"))])
async def add_chat_message(payload: dict, db: Session = Depends(get_db)):
    validation_list = [
    ...
    session_exists = await mysql_query_utils.get_one_field_detail(
        db, ChatSession, filters={"id": str(session_uuid)}
    )
    ...
    try:
        ...
        logger.info("#chat_message.py #add_chat_message #Message added successfully")
        return {"detail": "Message added successfully", "message_id": message_data["id"]}

    except Exception as e:
        logger.error(f"#chat_message.py #add_chat_message #Exception: {e}", exc_info=True)
